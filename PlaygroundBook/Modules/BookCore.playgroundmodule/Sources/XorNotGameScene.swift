...
public class XorNotGameScene: SKScene {
    ...
    let gate_XOR = XorGate(defaultScale: 0.31, draggingScale: 0.34)
    let gate_NOT = NotGate(defaultScale: 0.23, draggingScale: 0.26)
    
    let label = SKLabelNode()
    let labelText = "This circuit works like the XOR gate, but inverted!\nIt only works when all the signals are equal!"
    
    let engenheira = SKSpriteNode(imageNamed: "Engenheira")
    let winLabel = SKLabelNode()
    let winLabelText = "Well done!\nThe next gate is available.\n\nNext  ➡️"
    ...
    override public func didMove(to view: SKView) {
        ...
        gate_XOR.position.x = self.frame.midX - 100
        gate_XOR.position.y = self.frame.midY - 300
        gate_XOR.intersectionNode = gate_OUTLINE
        addChild(gate_XOR)
    ...
    func checkSolution() {
        if gate_XOR.isGateInPosition && !gate_NOT.isGateInPosition {
            gate_XOR.setGateState(.angry)
        } else if !gate_XOR.isGateInPosition && gate_NOT.isGateInPosition {
            gate_NOT.setGateState(.angry)
        }
        if gate_XOR.isGateInPosition && gate_NOT.isGateInPosition {
            if !emitterOutput1_ReachedEnd {
                gate_NOT.setGateState(.happy)
            }
            if !(energySwitch1.state == energySwitch2.state) && !(emitterInput1_ReachedEnd == emitterInput2_ReachedEnd) {
                if self.emitterNodeOutput1 == nil {
                    addEnergy(pos: CGPoint(x: 81, y: 100), lifetime: 0.34, parentEmitterNode: &self.emitterNodeOutput1)
                    
                    let energyEmitterOutput1_ReachedEnd = DispatchWorkItem {
                        if !(self.energySwitch1.state == self.energySwitch2.state) && !(self.emitterInput1_ReachedEnd == self.emitterInput2_ReachedEnd) && self.gate_XOR.isGateInPosition && self.gate_NOT.isGateInPosition {
                            self.emitterOutput1_ReachedEnd = true
                            self.setLedState(.OFF)
                            self.gate_XOR.setGateState(.happy)
                            self.gate_NOT.setGateState(.angry)
                            self.emitterNodeOutput2?.removeFromParent()
                            self.emitterNodeOutput2 = nil
                        }
                    }
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.40, execute: energyEmitterOutput1_ReachedEnd)
                }
                
                gate_XOR.setGateState(.happy)
            } else {
                if self.emitterNodeOutput2 == nil {
                    ...
                    let energyEmitterOutput2_ReachedEnd = DispatchWorkItem {
                        ...
                        if self.energySwitch1.state == self.energySwitch2.state && (self.gate_XOR.isGateInPosition && self.gate_NOT.isGateInPosition) {
                            self.setLedState(.ON)
                ...
                self.emitterOutput1_ReachedEnd = false
                self.gate_XOR.setGateState(.angry)
    ...
    func touchDown(atPoint pos : CGPoint) {
        gate_XOR.touchDown(atPoint: pos)
        ...
        if balaoFala.contains(pos) && balaoFala.alpha > 0.2 {
            if let nextGameScene = XnorGameScene(fileNamed: "GameScene") {
    ...
    func touchMoved(toPoint pos : CGPoint) {
        gate_XOR.touchMoved(toPoint: pos)
        gate_NOT.touchMoved(toPoint: pos)
        checkSolution()
    }

    func touchUp(atPoint pos : CGPoint) {
        gate_XOR.touchUp(atPoint: pos)
