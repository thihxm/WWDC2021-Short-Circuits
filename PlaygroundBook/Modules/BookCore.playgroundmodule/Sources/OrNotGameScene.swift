...
public class OrNotGameScene: SKScene {
    ...
    let gate_OR = OrGate(defaultScale: 0.31, draggingScale: 0.34)
    let gate_NOT = NotGate(defaultScale: 0.23, draggingScale: 0.26)
    
    let label = SKLabelNode()
    let labelText = "This circuit works like the OR gate, but inverted!\nIt only works when all the signals are off!"
    ...
    override public func didMove(to view: SKView) {
        ...
        gate_OR.position.x = self.frame.midX - 100
        gate_OR.position.y = self.frame.midY - 300
        gate_OR.intersectionNode = gate_OUTLINE
        addChild(gate_OR)
    ...
    func checkSolution() {
        if gate_OR.isGateInPosition && !gate_NOT.isGateInPosition {
            gate_OR.setGateState(.angry)
        } else if !gate_OR.isGateInPosition && gate_NOT.isGateInPosition {
            ...
            if (energySwitch1.state == .ON && emitterInput1_ReachedEnd) || (energySwitch2.state == .ON && emitterInput2_ReachedEnd) {
                if self.emitterNodeOutput1 == nil {
                    ...
                    let energyEmitterOutput1_ReachedEnd = DispatchWorkItem {
                        if (self.energySwitch1.state == .ON && self.emitterInput1_ReachedEnd) || (self.energySwitch2.state == .ON && self.emitterInput2_ReachedEnd) && self.gate_OR.isGateInPosition && self.gate_NOT.isGateInPosition {
                            self.emitterOutput1_ReachedEnd = true
                            self.setLedState(.OFF)
                            self.gate_OR.setGateState(.happy)
                ...
                if self.emitterNodeOutput2 == nil {
                    ...
                    let energyEmitterOutput2_ReachedEnd = DispatchWorkItem {
                        if (self.energySwitch1.state == .OFF || self.energySwitch2.state == .OFF) && (self.gate_OR.isGateInPosition && self.gate_NOT.isGateInPosition) {
                            self.setLedState(.ON)
                ...
                self.gate_OR.setGateState(.angry)
    ...
    func touchDown(atPoint pos : CGPoint) {
        ...
        if balaoFala.contains(pos) && balaoFala.alpha > 0.2 {
            if let nextGameScene = NorGameScene(fileNamed: "GameScene") {
    ...
    func touchMoved(toPoint pos : CGPoint) {
        gate_OR.touchMoved(toPoint: pos)
        gate_NOT.touchMoved(toPoint: pos)
        checkSolution()
    }

    func touchUp(atPoint pos : CGPoint) {
        gate_OR.touchUp(atPoint: pos)
