...
public class OrGameScene: SKScene {
    ...
    let labelText = "This is the OR Gate, it does not like when every energy signal is off.\nIt works when at least one signal is on!"
    
    let engenheira = SKSpriteNode(imageNamed: "Engenheira")
    let winLabel = SKLabelNode()
    let winLabelText = "Good job!\nThe next gate is available.\n\nNext  ➡️"
    ...
    override public func didMove(to view: SKView) {
        ...
        gate_OR.position.x = self.frame.midX
    ...
    func checkSolution() {
        if gate_OR.isGateInPosition {
            if (energySwitch1.state == .ON || energySwitch2.state == .ON) && (emitterInput1_ReachedEnd || emitterInput2_ReachedEnd) {
                if self.emitterNodeOutput == nil {
                    ...
                    let energyEmitterOutput_ReachedEnd = DispatchWorkItem {
                        if ((self.energySwitch1.state == .ON && self.emitterInput1_ReachedEnd) || (self.energySwitch2.state == .ON && self.emitterInput2_ReachedEnd)) && self.gate_OR.isGateInPosition {
                            self.setLedState(.ON)
    ...
    func touchDown(atPoint pos : CGPoint) {
        ...
        if balaoFala.contains(pos) && balaoFala.alpha > 0.2 {
            if let nextGameScene = AndNotGameScene(fileNamed: "GameScene") {
