...
public class XorGameScene: SKScene {
    ...
    let labelText = "This is the XOR Gate, its logic composition is a bit complex, but its usage no.\nIt only works when both input signals are different!"
    
    let engenheira = SKSpriteNode(imageNamed: "Engenheira")
    let winLabel = SKLabelNode()
    let winLabelText = "Nicely done!\nThe next gate is available.\n\nNext  ➡️"
    ...
    override public func didMove(to view: SKView) {
        ...
        gate_XOR.position.x = self.frame.midX
            ...
            if !(energySwitch1.state == energySwitch2.state) && !(emitterInput1_ReachedEnd == emitterInput2_ReachedEnd) {
                if self.emitterNodeOutput == nil {
                    ...
                    let energyEmitterOutput_ReachedEnd = DispatchWorkItem {
                        if !(self.energySwitch1.state == self.energySwitch2.state) && !(self.emitterInput1_ReachedEnd == self.emitterInput2_ReachedEnd) && self.gate_XOR.isGateInPosition {
                            self.setLedState(.ON)
    ...
    func touchDown(atPoint pos : CGPoint) {
        ...
        if balaoFala.contains(pos) && balaoFala.alpha > 0.2 {
            if let nextGameScene = XorNotGameScene(fileNamed: "GameScene") {
