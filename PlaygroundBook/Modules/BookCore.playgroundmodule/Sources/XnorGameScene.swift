...
public class XnorGameScene: SKScene {
    ...
    let gate_XNOR = XnorGate(defaultScale: 0.31, draggingScale: 0.34)
    
    let label = SKLabelNode()
    let labelText = "This is the XNOR Gate, it is the last circuit in one Gate.\nIt negates the result of the XOR Gate, so it only works when both input signals are equal!"
    
    let engenheira = SKSpriteNode(imageNamed: "Engenheira")
    let winLabel = SKLabelNode()
    let winLabelText = "Excellent!\nThis was the last gate available.\n\nNext  ➡️"
    ...
    override public func didMove(to view: SKView) {
        ...
        gate_XNOR.position.x = self.frame.midX
        gate_XNOR.position.y = self.frame.midY - 300
        gate_XNOR.intersectionNode = gate_OUTLINE
        addChild(gate_XNOR)
            ...
            if energySwitch1.state == energySwitch2.state && emitterInput1_ReachedEnd == emitterInput2_ReachedEnd {
                if self.emitterNodeOutput == nil {
                    ...
                    let energyEmitterOutput_ReachedEnd = DispatchWorkItem {
                        if (self.energySwitch1.state == self.energySwitch2.state && self.emitterInput1_ReachedEnd == self.emitterInput2_ReachedEnd) && self.gate_XNOR.isGateInPosition {
                            self.setLedState(.ON)
                ...
                gate_XNOR.setGateState(.happy)
            } else {
                setLedState(.OFF)
                gate_XNOR.setGateState(.angry)
    ...
    func touchDown(atPoint pos : CGPoint) {
        gate_XNOR.touchDown(atPoint: pos)
        ...
        if balaoFala.contains(pos) && balaoFala.alpha > 0.2 {
            if let nextGameScene = EndGameScene(fileNamed: "EndGameScene") {
    ...
    func touchMoved(toPoint pos : CGPoint) {
        gate_XNOR.touchMoved(toPoint: pos)
    }

    func touchUp(atPoint pos : CGPoint) {
        gate_XNOR.touchUp(atPoint: pos)
