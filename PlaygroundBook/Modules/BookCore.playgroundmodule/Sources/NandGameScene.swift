...
public class NandGameScene: SKScene {
    ...
    let gate_NAND = NandGate(defaultScale: 0.31, draggingScale: 0.34)
    
    let label = SKLabelNode()
    let labelText = "This is the NAND Gate, it is the last circuit in one Gate.\nIt negates the result of the AND Gate, so it works when at least one signal is off!"
    ...
    override public func didMove(to view: SKView) {
        ...
        gate_NAND.position.x = self.frame.midX
        gate_NAND.position.y = self.frame.midY - 300
        gate_NAND.intersectionNode = gate_OUTLINE
        addChild(gate_NAND)
    ...
    func checkSolution() {
        if gate_NAND.isGateInPosition {
            if (energySwitch1.state == .ON && energySwitch2.state == .ON) {
                setLedState(.OFF)
                gate_NAND.setGateState(.angry)
                ...
                if self.emitterNodeOutput == nil {
                    ...
                    let energyEmitterOutput_ReachedEnd = DispatchWorkItem {
                        if (self.energySwitch1.state == .OFF || self.energySwitch2.state == .OFF) && self.gate_NAND.isGateInPosition {
                            self.setLedState(.ON)
    ...
    func touchDown(atPoint pos : CGPoint) {
        gate_NAND.touchDown(atPoint: pos)
        ...
        if balaoFala.contains(pos) && balaoFala.alpha > 0.2 {
            if let nextGameScene = OrNotGameScene(fileNamed: "GameScene") {
    ...
    func touchMoved(toPoint pos : CGPoint) {
        gate_NAND.touchMoved(toPoint: pos)
    }

    func touchUp(atPoint pos : CGPoint) {
        gate_NAND.touchUp(atPoint: pos)
