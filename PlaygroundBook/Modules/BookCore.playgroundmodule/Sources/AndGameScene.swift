...
public class AndGameScene: SKScene {
    ...
    let labelText = "This is the AND Gate, it does not like off energy signals\nand it only works when all the signals are on!"
    
    let engenheira = SKSpriteNode(imageNamed: "Engenheira")
    let winLabel = SKLabelNode()
    let winLabelText = "Let's go!\nThe next gate is available.\n\nNext  ➡️"
    ...
    override public func didMove(to view: SKView) {
        ...
        gate_AND.position.x = self.frame.midX
            ...
            if (energySwitch1.state == .ON && energySwitch2.state == .ON) && (emitterInput1_ReachedEnd && emitterInput2_ReachedEnd) {
                if self.emitterNodeOutput == nil {
                    ...
                    let energyEmitterOutput_ReachedEnd = DispatchWorkItem {
                        if (self.energySwitch1.state == .ON && self.emitterInput1_ReachedEnd) && (self.energySwitch2.state == .ON && self.emitterInput2_ReachedEnd) && self.gate_AND.isGateInPosition {
                            self.setLedState(.ON)
    ...
    func touchDown(atPoint pos : CGPoint) {
        ...
        if balaoFala.contains(pos) && balaoFala.alpha > 0.2 {
            if let nextGameScene = OrGameScene(fileNamed: "GameScene") {
